...
export class UIStateBuilder {
  ...
  public build(): any {
    ...
    if (playerUIConfig) {
      // First, interpolate the entire player UI config with the global context.
      // This resolves non-player-specific templates like {{gameState...}} or {{metadata...}}.
      const interpolatedConfig = this.valueResolver.interpolate(playerUIConfig) as UIConfiguration;
      ...
      players.forEach((player: Player) => {
        // Now, resolve the player-specific parts (like conditions) for each player.
        playerUIs[player.id] = this.resolvePlayerUI(player, interpolatedConfig);
      });
    }
    ...
    // Final interpolation pass to resolve any remaining cross-component references.
    return this.valueResolver.interpolate(finalState);
  }
}
