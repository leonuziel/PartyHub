...
export class EffectExecutor {
  ...
  public execute(effectOrEffects: any, context: any = {}) {
    ...
    for (const effect of effects) {
      ...
      if (effect.forEachPlayer) {
        this.executeForEachPlayer(effect.forEachPlayer.effects, context);
        continue;
      }
  ...
  private executeForEachPlayer(effects: any[], context: any) {
    this.valueResolver['fullContext'].players.forEach((player: any) => {
      const playerState = {
        ...player,
        ...(this.gameState.playerAttributes?.[player.id] || {})
      };
      const playerContext = { ...context, player: playerState };
  ...
  private dispatchEffect(funcName: string, args: any[]) {
    switch (funcName) {
      case 'startTimer':
        setTimeout(() => this.handleInternalAction('timerExpires'), args[0] * 1000);
        break;
      case 'setProperty':
