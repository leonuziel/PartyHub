...
describe('VotingGrid', () => {
  ...
  it('renders the options correctly', () => {
    renderWithProviders(<VotingGrid options={mockOptions} onVote={onVote} />);
    expect(screen.getByText('Option 1')).toBeInTheDocument();
    expect(screen.getByRole('img')).toBeInTheDocument();
  });

  it('calls onVote with the correct option ID when an option is clicked', async () => {
    renderWithProviders(<VotingGrid options={mockOptions} onVote={onVote} />);
  ...
  it('disables the grid and shows a "Voted" indicator after a vote is cast', async () => {
    renderWithProviders(<VotingGrid options={mockOptions} onVote={onVote} />);
    ...
    await userEvent.click(option1Card!);
    ...
    // Try to vote again
    await userEvent.click(option2Card!);
    expect(onVote).toHaveBeenCalledTimes(1); // Should not have been called again
  });

  it('does not allow voting if disabled prop is true', async () => {
    renderWithProviders(<VotingGrid options={mockOptions} onVote={onVote} disabled={true} />);
    const option1 = screen.getByText('Option 1');
    await userEvent.click(option1);
